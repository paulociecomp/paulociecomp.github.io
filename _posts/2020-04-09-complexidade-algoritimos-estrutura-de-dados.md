...
Computação é sobre manipular dados. Um dos grandes desafios que temos como desenvolvedores é descobrir formas eficientes de lidar com esses dados. Estrutura de dados são maneiras de organizar esses dados. Vamos ver que, a maneira como organizamos esses dados, causa impacto na performance de nossos programas. Uma função lenta que executada só uma vez, talvez não cause problemas, mas quando ela for executada por milhares de usuários de uma aplicação web, podemos ter dores de cabeça.
...
Temos um array de 5 strings. O index é um número que identifica onde um determinado elemento está posicionado no array. Geralmente o index começa por 0. Então o elemento de index 0 é a ipa, o de index 1 é a apa, e assim por diante.
...
Olhando para o array de 5 elementos, podemos identificar facilmente quais são esses elementos e onde estão posicionados, mas o computador não possui olhos, então ele não sabe nada disso. O que o computador conhece é onde o array começa e qual o seu tamanho.
...
Para inserir um elemento no final do array é muito simples. O computador sabe onde o array começa e sabe qual o tamanho do array. Então ele calcula para saber em qual posição da memória ele irá inserir o novo elemento e ele faz isso em apenas uma etapa.
...
A pesquisa linear é mais eficiente em um array ordenado do que em um array comum. No entanto, de nada adianta se o elemento pesquisado estiver na última posição do array. Para este cenário precisamos pensar em algoritmos mais performáticos. E temos um muito bom.
...
Aprendemos que velocidade não se mede por tempo e sim por etapas. Vimos algumas operações e seus respectivos graus de eficiência e arrays. E percebemos que não só o tipo de estrutura de dados influencia na performance, mas também o algoritmo empregado para executar operações. No próximo post, vamos entender como funciona a análise propriamente dita da complexidade de algoritmos, sem blá blá blá acadêmico, usando a famigerada Notação O, o que meu professor com PHD falhou miseravelmente em me explicar, mas que consegui aprender lendo um simples tutorial na internet.
